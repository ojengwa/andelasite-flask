...
angular.module('core').controller('HireController', ['$scope', '$http', '$location', 'EmailService',
    ...
    $scope.hireDeveloper = function() {
      ...
      $http({
          ...
          $scope.loading = false;
          $scope.credentials = '';
          $scope.success = 'Request Successful!';
          $scope.hireDeveloperForm.$setPristine();
    ...
    $scope.credentials = EmailService;
    // JQuery functions
    $('.input-group .form-control').focus(function() {
        $(this).prev().parents('.input-group').addClass('colored-fields');
        $(this).prev().parents('.input-group').siblings().removeClass('colored-fields');
    });

}])
  .directive('country', ['$parse', function ($parse) {
    var countries = [
    ...
    return {
      restrict: 'E',
      template: '<select><option value="">Select Country</option><option>' + countries.join('</option><option>') + '</option></select>',
      replace: true,
